  ...
  request: Object.freeze({
    ...
    get body() {
      ...
      if (store.request.body && typeof store.request.body === 'string') {
        ...
        const items = Object.entries(store.request.data).map(([key, value]) => ({
          key: key,
          value: value,
        }));
  ...
  variables: Object.freeze({
    ...
    replaceIn(template) {
      if (dynamicGeneratorsRegex.test(template)) {
        ...
        return template.replace(dynamicGeneratorsRegex, (match) => {
      ...
      if (expression.variables && expression.variables.test(template)) {
...
const demarshal = Object.freeze({
  integer(encoded) {
    ...
    return Number.parseInt(JSON.stringify(encoded));
  },
});
