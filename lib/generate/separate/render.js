...
function render(mapped, result) {
  return location(mapped, result, 1, null);
}

function location(node, result, depth, upstreamAuth) {
  const safeDepth = depth ?? 1;
  const safeUpstreamAuth = upstreamAuth ?? null;
  const auth = node[Auth] || safeUpstreamAuth;
  const tree = {};
  entries(node, tree, safeDepth, auth, result);
  return tree;
}
...
function entry(node, depth, auth, result) {
  ...
  if (typeof node === 'object') {
    return location(node, result, depth + 1, auth);
  }
