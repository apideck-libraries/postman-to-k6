...
function address(address, feature, result) {
  ...
  if (aid.variableStart.test(addressText)) {
    ...
    if (result.setting?.separate && result.setting.separate === true) {
      // Request is in a Postman folder
      const depthLevel = result.group?.level ? result.group.level + 1 : 1; // +1 starting in the "requests folder
      const pathDepth = '../'.repeat(depthLevel);
      urijsPath = `${pathDepth}libs/shim/urijs.js`;
    }
  ...
  feature.address = feature.address.query(
    feature.address.query().replace(/\s/g, '%20')
  );
}
