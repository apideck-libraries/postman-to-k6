...
function globalScope(result) {
  if (result.scope.global.size) {
    // Replace environment variables with environment or collection values
    result.scope.global.forEach((valObj, key, map) => {
      const value = valObj.value;

      // if value starts with double curly braces and ends with double curly braces
      if (
        value &&
        ...
        // search for varName in Map of environments scope
        const varEnvObj = result.scope.environment.get(varName);
        if (varEnvObj) {
          // Update key with varEnvObj
          map.set(key, varEnvObj);
          return;
        }

        // search for varName in Map of collection scope
        const varCollObj = result.scope.collection.get(varName);
        if (varCollObj) {
          // Update key with varCollObj
          map.set(key, varCollObj);
        }
      }
    });

    // Render     return `global: ${vars(result.scope.global)}`; scope
...
function collectionScope(result) {
  if (result.scope.collection.size) {
    // Replace environment variables with environment or global values
    result.scope.collection.forEach((valObj, key, map) => {
      const value = valObj.value;
      ...
      if (
        ...
        // search for varName in Map of global scope
        const varGlobalObj = result.scope.global.get(varName);
        if (varGlobalObj) {
          // Update key with varGlobalObj
          map.set(key, varGlobalObj);
        }
      }
    });

    // Render collection scope
...
function environmentScope(result) {
  if (result.scope.environment.size) {
    // Replace environment variables with global or collection values
    result.scope.environment.forEach((valObj, key, map) => {
      const value = valObj.value;
